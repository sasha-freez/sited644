...
*/
$text = <<<TEXT
Данные родителей, которые дают согласие на выезд ребёнка => titul[EOL]
ФИО => text=40,*[EOL]
...
Место жительства (по паспорту) => text=40,*[EOL]
=> [EOL]
Данные ребёнка => titul[EOL]
ФИО => text=40,*[EOL]
Дата рождения => text=40,*[EOL]
=> [EOL]
Данные о выезде => titul[EOL]
Страна (страны), которую намерен посетить ребёнок => text=40,*[EOL]
Срок (период) выезда ребёнка => text=40,*[EOL]
ФИО сопровождающего лица (лиц) => text=40,*[EOL]
=> [EOL]
...
/* Начало обработки формы */
	...
	$m=0; $ms=0;
	while ($m<5){if (mail($mailto, $mailtheme, nl2br($message), $headers)) {$m=5; $ms=1;} else {$m++;}}
	if ($ms){echo '<b style="font-size:14px; color:red;">Сообщение отправлено!</b><br />';} else {echo '<b style="font-size:14px; color:red;"><u>К сожалению, сообщение не отправлено даже с 5ой попытки. <a href="mailto:'.$mailto.'" target="_blank">Напишите, пожалуйста, нам письмо.</a></u></b><br />';}
	echo nl2br($message);
