...
3. select - Поле выбора, в качестве параметров идут: высота поля, [значения полей/значения].
		  (
		  Пример (высота 1):
		  Название => select=1,[значени1/значение2/значение3][EOL]
		  )
4. любое другое значение - будет расцениваться как пустая строка. Желательно употреблять так: =>[EOL]
*/
$text = <<<TEXT
Данные супруга,  который даёт согласие => titul[EOL]
ФИО => text=40,*[EOL]
...
=>[EOL]
Данные супруга, которому выдают согласие => titul[EOL]
ФИО => text=40,*[EOL]
=>[EOL]
Данные объекта, который продается (покупается) => titul[EOL]
Адрес объекта => text=40,*[EOL]
Объект => select=1,[продается/покупается][EOL]
=>[EOL]
Данные свидетельства о заключении брака => titul[EOL]
Серия и номер свидетельства => text=40,*[EOL]
...
/* Начало обработки формы */
	...
	if (!mail($mailto, $mailtheme, nl2br($message), $headers)) {
		echo '<b style="font-size:14px; color:red;"><u>К сожалению, сообщение не отправлено. <a href="javascript:history.back();">Попробуйте еще раз.</a></u></b><hr />'.nl2br($headers).'<hr/>'.nl2br($message).'<hr/>'; die();} else {echo '<b style="font-size:14px; color:red;">Сообщение отправлено!</b><br />';
	}
	echo nl2br($message);
...
for($j=0; $j<count($array); $j++) {
	...
	else if (preg_match('/^(select)/', $key)) {
		$param = substr($key,strpos($key,'=')+1);
		if (strpos($param, ',') > 0) {$keyarr = explode(',',$param); if(count($keyarr)==2) {$keyarr[2]='';}}
		$keyvals = substr($keyarr[1],strpos($keyarr[1],'[')+1,-1);
		$keyvals = explode('/',$keyvals);
		echo "<tr><td class='f_name'>$value: ";
		echo "</td><td class='f_form'><select name='".translit($value).$j."' size='".$keyarr[0]."'>";
		for ($k=0; $k<count($keyvals); $k++) {echo "<option value='".$keyvals[$k]."'>".$keyvals[$k]."</option>";}
		echo "</select>";
	}
